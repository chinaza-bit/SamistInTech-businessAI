...
if (currentPage) {
  document.querySelectorAll(`.main-nav a[data-page="${currentPage}"]`).forEach(a => a.classList.add("active"));
}
...
if (heroEl) {
  heroEl.addEventListener("mousemove", (e) => {
    ...
    heroEl.style.setProperty("--sx", ((e.clientX - rect.left) / rect.width) * 100 + "%");
    heroEl.style.setProperty("--sy", ((e.clientY - rect.top) / rect.height) * 100 + "%");
  });
}
...
function blogCard(post) {
  const cover = post.cover_image_url
    ...
    : ``;
  return `
    <a class="blog-card reveal" href="post.html?slug=${encodeURIComponent(post.slug)}">
      <div class="cover-wrap">${cover}</div>
      <div class="body">
        <div class="meta-row">
          <p class="date">${formatDate(post.published_at)}</p>
          <span class="read-time">${readTime(post.content || post.excerpt)} min read</span>
        </div>
        <h3>${escapeHtml(post.title)}</h3>
        <p class="excerpt">${escapeHtml(post.excerpt)}</p>
        <span class="read-more">Read post <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg></span>
      </div>
    </a>`;
}
  ...
  if (!slug) {
    ...
    db.from("blog_posts")
      ...
      .then(({ data, error }) => {
        ...
        headEl.innerHTML = `<p class="date">${formatDate(data.published_at)} <span class="read-time" style="margin-left:6px;">${readTime(data.content)} min read</span></p><h1>${escapeHtml(data.title)}</h1>`;
...
// ---------- WhatsApp channel promo bar ----------
const waPromo = document.getElementById("waPromo");
if (waPromo) {
  const DISMISS_KEY = "sit_wa_promo_dismissed_until";
  const dismissedUntil = Number(localStorage.getItem(DISMISS_KEY) || 0);
  if (Date.now() > dismissedUntil) {
    setTimeout(() => waPromo.classList.add("show"), 1500);
  }
  const closeBtn = document.getElementById("waPromoClose");
  closeBtn.addEventListener("click", () => {
    waPromo.classList.remove("show");
